...
- **Wait-for-client on disconnect** — When a permission prompt is raised while no clients are connected to the session, the server does *not* immediately deny it. The prompt is broadcast globally and replayed when a client joins. It is still bounded by the 5-minute auto-deny timeout below.
- **Approval countdown with auto-approve** — Permission prompts that offer an "Always Allow" option display a 15-second countdown timer in the UI. When the timer expires, the tool is automatically approved. This prevents sessions from stalling on routine approvals when the user is away. The countdown does *not* run for prompts that require an explicit decision (e.g. plan-approval, question prompts), which omit the "Always Allow" option.
...
- **5-minute auto-deny safety timeout** — If a pending tool approval (or plan approval) is not answered within 5 minutes, the server auto-**denies** it to prevent leaked promises and indefinitely stuck processes. There is no server-side auto-approve.
