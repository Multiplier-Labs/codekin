...
export function RepoList({ groups, selectedId, onSelect, cloningId, maxHeight = '240px', autoFocus }: Props) {
  ...
  if (totalRepos === 0) {
    return <p className="text-center text-body text-ink-faint py-2">No repositories available</p>
  }
  ...
  return (
    <>
      <input
        ...
        className="mb-2 w-full rounded-control border border-edge-strong bg-surface/50 px-3 py-2 text-body text-ink placeholder-ink-muted focus:border-focus focus:outline-none"
      />
      <div className="overflow-y-auto rounded-control border border-edge-strong bg-surface/50" style={{ maxHeight }}>
                  ...
                  <button
                    ...
                    className={`group flex w-full items-start gap-3 border-b border-edge-strong px-3 py-2.5 text-left transition last:border-b-0 ${
                      isSelected
                        ...
                        : cloningId ? 'cursor-wait opacity-60' : 'hover:bg-edge'
                    }`}
                  >
                    <div className="mt-0.5 flex-shrink-0 text-ink-muted group-hover:text-ink transition-colors">
                      {repo.cloned ? (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-body font-medium text-ink group-hover:text-ink transition-colors truncate">
                          {repo.name}
                        </span>
                        {isCloning && (
                        ...
                        {!repo.cloned && !isCloning && (
                          <span className="flex-shrink-0 rounded-control border border-ink-faint bg-edge-strong/50 px-1.5 py-0.5 text-meta text-ink">
                            remote
                          </span>
                        )}
                      </div>
                      {repo.description && (
                        <div className="mt-0.5 text-meta text-ink-faint truncate">
                          {repo.description}
                        </div>
                      )}
                    </div>
