...
export function ModuleBrowser({ repo, globalModules = [], onSendModule, disabled }: Props) {
  ...
  return (
    <div className="mt-3 border-t border-neutral-10 pt-3">
      <h3 className="mb-2 text-body font-medium uppercase tracking-wider text-neutral-5">Modules</h3>

      {globalModules.length > 0 && (
        <>
          <div className="mb-1 px-2 text-meta font-medium uppercase tracking-wider text-neutral-5">Global</div>
          <div className="flex flex-col gap-1">
            {globalModules.map(mod => (
              ...
              >
                ...
                {mod.description && (
                  <div className="text-body text-neutral-4 group-hover:text-neutral-4">
                    {mod.description}
                  </div>
                )}
              </button>
      ...
      {repoModules.length > 0 && (
        <>
          <div className="mb-1 mt-2 px-2 text-meta font-medium uppercase tracking-wider text-neutral-5">{repo?.name}</div>
          <div className="flex flex-col gap-1">
            {repoModules.map(mod => (
              <button
                key={mod.id}
