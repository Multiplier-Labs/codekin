/**
 * Sidebar panel for browsing and attaching context modules.
 *
 * Modules are reusable text blocks (instructions, prompts, context)
 * that can be injected into a chat message. Displays both global
 * modules and repo-specific modules when a repo is active.
 */

import type { Module, Repo } from '../types'

interface Props {
  repo: Repo | null
  globalModules?: Module[]
  onSendModule: (module: Module) => void
  disabled?: boolean
}

export function ModuleBrowser({ repo, globalModules = [], onSendModule, disabled }: Props) {
  const repoModules = repo?.modules ?? []
  if (globalModules.length === 0 && repoModules.length === 0) return null

  return (
    <div className="mt-3 border-t border-edge pt-3">
      <h3 className="mb-2 text-body font-medium uppercase tracking-wider text-ink-muted">Modules</h3>

      {globalModules.length > 0 && (
        <>
          <div className="mb-1 px-2 text-meta font-medium uppercase tracking-wider text-ink-muted">Global</div>
          <div className="flex flex-col gap-1">
            {globalModules.map(mod => (
              <button
                key={mod.id}
                onClick={() => onSendModule(mod)}
                disabled={disabled}
                className="group rounded-control px-2 py-1.5 text-left transition hover:bg-surface-raised disabled:opacity-50 disabled:pointer-events-none"
              >
                <div className="text-body font-medium text-primary-6 group-hover:text-primary-5">
                  {mod.name}
                </div>
                {mod.description && (
                  <div className="text-body text-ink-muted group-hover:text-ink-muted">
                    {mod.description}
                  </div>
                )}
              </button>
            ))}
          </div>
        </>
      )}

      {repoModules.length > 0 && (
        <>
          <div className="mb-1 mt-2 px-2 text-meta font-medium uppercase tracking-wider text-ink-muted">{repo?.name}</div>
          <div className="flex flex-col gap-1">
            {repoModules.map(mod => (
              <button
                key={mod.id}
                onClick={() => onSendModule(mod)}
                disabled={disabled}
                className="group rounded-control px-2 py-1.5 text-left transition hover:bg-surface-raised disabled:opacity-50 disabled:pointer-events-none"
              >
                <div className="text-body font-medium text-primary-6 group-hover:text-primary-5">
                  {mod.name}
                </div>
                {mod.description && (
                  <div className="text-body text-ink-muted group-hover:text-ink-muted">
                    {mod.description}
                  </div>
                )}
              </button>
            ))}
          </div>
        </>
      )}
    </div>
  )
}
